...
import { Transcript, Summary, SummaryDocument } from "@/types";
...
function MeetingDetailsContent() {
  ...
  const [meetingSummary, setMeetingSummary] = useState<SummaryDocument | null>(
    null,
  );
  ...
  // Check whether a packaged local summary model is ready. This never
  // contacts Ollama or any remote model service.
  const getAvailableLocalSummaryModel = useCallback(async (): Promise<
    string | null
  > => {
    try {
      ...
      console.error("Failed to check packaged local models:", error);
      return null;
    }
  }, []);
  ...
  const setupAutoGeneration = useCallback(async () => {
    ...
    try {
      ...
      // DB is empty - use the highest-priority packaged local model that is ready.
      const availableLocalModel = await getAvailableLocalSummaryModel();

      if (availableLocalModel) {
        console.log(
          "Packaged local summary model is ready:",
          availableLocalModel,
        );
        ...
        console.log("⚠️ No packaged local summary model is ready");
      }
    } catch (error) {
