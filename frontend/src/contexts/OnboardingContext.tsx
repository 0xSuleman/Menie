...
export function OnboardingProvider({
  ...
  const initializeSummaryModelSelection = async (
    ...
    try {
      const profiles = await invoke<LocalModelProfile[]>(
        "get_local_model_profiles",
      );
      const recommendedModel = profiles.find(
        (profile) => profile.recommended,
      )?.id;
      if (!recommendedModel) {
        throw new Error(
          "Menie could not recommend a local model for this device.",
        );
      }
  ...
  const performAutoDetection = async () => {
    ...
    if (
      ...
      const homebrewDbPath = "/usr/local/var/menie/meeting_minutes.db";
      try {
