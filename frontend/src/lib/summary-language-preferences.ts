...
const DETECTED_SUMMARY_LANGUAGE_FALLBACK_PREFIX =
  "detectedSummaryLanguageFallback";
...
type RawMeetingSummaryLanguagePreference =
  MeetingSummaryLanguagePreference | string | null;
...
export function readPinnedSummaryLanguageDefault(): string | null {
  ...
  try {
    return normaliseLanguageCode(
      window.localStorage.getItem(SUMMARY_LANGUAGE_DEFAULT_KEY),
    );
  } catch {
    return null;
  }
...
export async function readMeetingSummaryLanguage(
  ...
  if (response.storage === "local_fallback") {
    return {
      language: readLanguageFallback(
        SUMMARY_LANGUAGE_FALLBACK_PREFIX,
        meetingId,
      ),
...
export async function readCachedDetectedSummaryLanguage(
  meetingId: string,
): Promise<string | null> {
  const response = normalisePreferenceResponse(
    await invoke<RawMeetingSummaryLanguagePreference>(
