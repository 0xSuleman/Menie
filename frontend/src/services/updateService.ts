...
export class UpdateService {
  ...
  async assertInstallationAllowed(): Promise<void> {
    const recording = await invoke<boolean>("is_recording");
    if (recording) {
      throw new Error(
        "Updates are paused while recording is active. Stop the recording and try again.",
      );
    }
  }
  ...
  async downloadAndInstall(
    ...
    try {
      // Never interrupt capture or force a relaunch while meeting data is
      // being collected. The native command is the authoritative state.
      await this.assertInstallationAllowed();
