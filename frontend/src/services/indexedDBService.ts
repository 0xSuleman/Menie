...
class IndexedDBService {
  ...
  private readonly DB_NAME = "MenieRecoveryDB";
      ...
      try {
        ...
        request.onupgradeneeded = (event) => {
          ...
          if (!db.objectStoreNames.contains("meetings")) {
            ...
            meetingsStore.createIndex("lastUpdated", "lastUpdated", {
              unique: false,
            });
          ...
          if (!db.objectStoreNames.contains("transcripts")) {
            ...
            transcriptsStore.createIndex("storedAt", "storedAt", {
              unique: false,
            });
          }
        };
  ...
  async deleteOldMeetings(daysOld: number): Promise<number> {
    try {
      ...
      // Get all meetings
      const allMeetings = await new Promise<MeetingMetadata[]>(
        (resolve, reject) => {
