...
/** The channel is deliberately local-only; it is never fetched from a server. */
export function getReleaseChannel(): ReleaseChannel {
  if (typeof window === "undefined") return "stable";
  const value = localStorage.getItem("menie.releaseChannel");
  return value === "beta" || value === "dev" ? value : "stable";
}

export function setReleaseChannel(channel: ReleaseChannel): void {
  if (typeof window !== "undefined") {
    localStorage.setItem("menie.releaseChannel", channel);
  }
}

export function resetBetaFeatures(): BetaFeatures {
  const defaults = { ...DEFAULT_BETA_FEATURES };
  saveBetaFeatures(defaults);
  return defaults;
}
