...
import { applyRememberedSourceLabels } from "@/lib/sourceLabelMemory";

type RecordingMarker = { offset_seconds: number; text: string };
...
export function useRecordingStop(
      ...
      try {
        ...
        unlistenFn = await listen<{
          ...
          recordingStoppedDataRef.current = (async () => {
            ...
            if (event.payload.markers) {
              sessionStorage.setItem(
                "last_recording_markers",
                JSON.stringify(event.payload.markers),
              );
            }
  ...
  const handleRecordingStop = useCallback(
    async (isCallApi: boolean) => {
      ...
      try {
        const recordingPreferences = await invoke<{ record_only?: boolean }>(
          "get_recording_preferences",
        ).catch(() => ({ record_only: false }));
        const recordOnly = recordingPreferences.record_only === true;
        ...
        if (isCallApi && transcriptionComplete == true) {
          ...
          // Apply only explicit, device-local source labels chosen by the user.
          // The underlying capture provenance remains deterministic and no
          // speaker identity is inferred from audio.
          const freshTranscripts = applyRememberedSourceLabels([
          ...
          try {
            ...
            const rawMarkers = sessionStorage.getItem("last_recording_markers");
            if (rawMarkers) {
              try {
                const markers = JSON.parse(rawMarkers) as RecordingMarker[];
                if (markers.length > 0) {
                  await storageService.saveRecordingMarkers(meetingId, markers);
                }
              } catch (error) {
                console.warn(
                  "Meeting saved, but recording markers could not be persisted:",
                  error,
                );
                toast.warning(
                  "Meeting saved without one or more recording markers",
                );
              }
            }

            if (recordOnly && folderPath) {
              try {
                await invoke("enqueue_deferred_transcription", {
                  payload: {
                    meeting_id: meetingId,
                    recording_folder: folderPath,
                  },
                });
                toast.info("Recording saved for deferred transcription", {
                  description:
                    "You can process this local recording when you are ready.",
                });
              } catch (error) {
                console.warn(
                  "Recording saved, but deferred transcription was not queued:",
                  error,
                );
                toast.warning(
                  "Recording saved without a deferred transcription job",
                  {
                    description:
                      "You can still import the audio later from the local recording folder.",
                  },
                );
              }
            }
            ...
            try {
              shouldDetectSummaryLanguage =
                !(await applyPinnedSummaryLanguageToMeeting(meetingId));
            } catch (error) {
              console.warn(
                "Failed to apply pinned summary language preference for new meeting:",
                error,
              );
              toast.warning("Could not apply default summary language", {
                description:
                  "The meeting was saved, but the default summary language was not applied.",
              });
            }
            ...
            if (shouldDetectSummaryLanguage) {
              try {
                ...
                console.warn(
                  "Failed to detect summary language for new meeting:",
                  error,
                );
                toast.warning("Could not detect summary language", {
                  description:
                    "The meeting was saved, but Auto could not detect the summary language.",
                });
              }
            }
            ...
            sessionStorage.removeItem("last_recording_markers");
            ...
            try {
              ...
              if (
                freshTranscripts.length > 0 &&
                freshTranscripts[0].audio_start_time !== undefined
              ) {
                // Use audio_end_time of last transcript if available
                const lastTranscript =
                  freshTranscripts[freshTranscripts.length - 1];
                durationSeconds =
              ...
              const wordsPerMinute =
                durationSeconds > 0
                  ? transcriptWordCount / (durationSeconds / 60)
                  : 0;
              ...
              if (totalMeetings === 1) {
                const daysSinceInstall =
                  await Analytics.calculateDaysSince("first_launch_date");
              ...
              console.error(
                "Failed to track meeting completion analytics:",
                analyticsError,
              );
