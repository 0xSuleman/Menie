...
export function useImportAudio({
  ...
  const selectFile = useCallback(async (): Promise<AudioFileInfo | null> => {
    ...
    try {
      const result = await invoke<AudioFileInfo | null>(
      ...
      const errorMsg =
        typeof err === "string"
          ? err
          : err?.message || String(err) || "Failed to validate file";
      setError(errorMsg);
  ...
  const validateFile = useCallback(
    async (path: string): Promise<AudioFileInfo | null> => {
      ...
      try {
        const result = await invoke<AudioFileInfo>(
    ...
    async (
      ...
      try {
        ...
        const errorMsg =
          typeof err === "string"
            ? err
            : err?.message || String(err) || "Failed to start import";
        setError(errorMsg);
