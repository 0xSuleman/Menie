  ...
  try {
    ...
    if (detection.reason === "tie") {
      toast.warning("Bilingual transcript detected", {
        description:
          "Pick a summary language manually if Auto chooses the wrong fallback.",
      });
    }
...
interface UseSummaryGenerationProps {
  ...
  setAiSummary: (summary: SummaryDocument | null) => void;
...
export function useSummaryGeneration({
        ...
        startSummaryPolling(meeting.id, process_id, async (pollingResult) => {
          ...
          if (pollingResult.status === "cancelled") {
            ...
            try {
              ...
              console.error(
                "Failed to reload summary after cancellation:",
                error,
              );
              setSummaryStatus("idle");
            }
            ...
            toast.error(
              `Failed to ${isRegeneration ? "regenerate" : "generate"} summary`,
              {
                description: errorMessage.includes("Connection refused")
                  ? "Could not start the packaged local AI runtime. Confirm that a local summary model is installed, then try again."
                  : errorMessage,
              },
            );
          ...
          if (pollingResult.status === "completed" && pollingResult.data) {
            ...
            if (pollingResult.data.markdown) {
              ...
              setAiSummary({
                ...
                outcomes: pollingResult.data.outcomes || [],
            ...
            const allEmpty = summarySections.every(
              ([, section]) =>
                !(section as any).blocks ||
                (section as any).blocks.length === 0,
            );
  ...
  const fetchAllTranscripts = useCallback(
    async (meetingId: string): Promise<Transcript[]> => {
      try {
        ...
        console.log(
          `✅ Fetched ${allData.transcripts.length} transcripts from database`,
        );
    ...
    (allTranscripts: Transcript[]) => {
      ...
      return {
        transcriptText: allTranscripts
          .map(
            (t) => `${formatTime(t.audio_start_time, t.timestamp)} ${t.text}`,
          )
          .join("\n"),
    ...
    async (customPrompt: string = "") => {
      ...
      if (modelConfig.provider === "builtin-ai") {
        try {
          ...
          const isReady = await invokeTauri<boolean>(
  ...
  const handleStopGeneration = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "✓ Backend cancellation request sent for meeting:",
        meeting.id,
      );
    } catch (error) {
