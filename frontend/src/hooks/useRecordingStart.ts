...
import { hasAcknowledgedRecordingNotice } from "@/lib/recordingNotice";
...
export function useRecordingStart(
  ...
  const checkParakeetReady = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (recordingPreferences.record_only) {
        // Record Only is a native capture mode: no ASR model is required or started.
        return true;
      }
  ...
  const handleRecordingStart = useCallback(async () => {
    try {
      ...
      if (!hasAcknowledgedRecordingNotice()) {
        toast.info(
          "Before recording, acknowledge the participant notice in the recording controls.",
        );
    ...
    const checkAutoStartRecording = async () => {
      if (typeof window !== "undefined") {
        ...
        if (shouldAutoStart === "true" && !isRecording && !isAutoStarting) {
          ...
          if (!hasAcknowledgedRecordingNotice()) {
            toast.info(
              "Automatic recording is waiting for your participant-notice acknowledgement.",
            );
  ...
  useEffect(() => {
    const handleDirectStart = async (event: Event) => {
      ...
      try {
        ...
        // Preserve the user-selected detection context in the meeting title.
        const generatedMeetingTitle =
          detectionDetail?.origin === "meeting-detection"
            ? `${detectionDetail.combined ? "Combined" : detectionDetail.appName || "Detected"} Meeting ${generateMeetingTitle().replace("Meeting ", "")}`
            : generateMeetingTitle();
