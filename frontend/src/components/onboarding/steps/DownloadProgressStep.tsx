...
export function DownloadProgressStep() {
  ...
  const handleRetrySummaryDownload = async () => {
    ...
    if (retryingSummaryRef.current) {
      console.log(
        "[DownloadProgressStep] Summary retry already in progress, ignoring",
      );
      return;
    }
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600">
              {state.downloadedMb.toFixed(1)} {sizeUnit} /{" "}
              {state.totalMb.toFixed(1)} {sizeUnit}
            </span>
  ...
  return (
    <OnboardingContainer
      ...
      description="You can start using Menie after downloading the Transcription Engine."
      step={3}
