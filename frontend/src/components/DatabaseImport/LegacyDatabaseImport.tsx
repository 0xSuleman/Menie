...
type ImportState =
  "idle" | "selecting" | "detecting" | "importing" | "success" | "error";

export function LegacyDatabaseImport({
  isOpen,
  onComplete,
}: LegacyDatabaseImportProps) {
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage(
          "No database found at selected location. Please select the Menie folder, backend folder, or the database file directly.",
        );
  ...
  const isLoading = ["selecting", "detecting", "importing"].includes(
    importState,
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-2xl">Welcome to Menie!</DialogTitle>
          <DialogDescription className="text-base pt-2">
            Do you have data from a previous Menie installation?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              Select your previous Menie folder, backend directory, or database
              file:
            </p>
