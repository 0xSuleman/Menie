          ...
          <>
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
              <span>
                ...
                {download.totalMb.toFixed(1)} {unitLabel}
              </span>
...
export function useDownloadProgressToast() {
  ...
  const [dismissedModels, setDismissedModels] = useState<Set<string>>(
    new Set(),
  );

  const updateDownload = useCallback(
    (modelName: string, data: Partial<DownloadProgress>) => {
      setDownloads((prev) => {
  ...
  const cleanupDownload = useCallback(
    (modelName: string, delay: number = 4000) => {
  ...
  const showDownloadToast = useCallback((download: DownloadProgress) => {
    ...
    toast.custom(
      (t) => (
        <DownloadToastContent download={download} onDismiss={dismissToast} />
      ),
      {
        position: "top-right",
        id: toastId,
  ...
  useEffect(() => {
    downloads.forEach((download) => {
      ...
      if (
        dismissedModels.has(download.modelName) &&
