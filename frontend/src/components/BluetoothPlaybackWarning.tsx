...
export function BluetoothPlaybackWarning({
  ...
  useEffect(() => {
    ...
    const checkAudioOutput = async () => {
      try {
        const outputInfo = await invoke<AudioOutputInfo>(
      ...
      <div className="flex items-start justify-between w-full">
        <div className="flex-1">
          ...
          <AlertDescription className="text-yellow-800 mt-1">
            You're using <strong>{deviceName}</strong> for playback. Recordings
            may sound distorted or sped up through Bluetooth devices. For
            accurate review, please use <strong>computer speakers</strong> or{" "}
            <strong>wired headphones</strong>.
            <br />
            <a
              href="https://github.com/0xSuleman/Menie/blob/main/BLUETOOTH_PLAYBACK_NOTICE.md"
              target="_blank"
