...
export function ModelSettingsModal({
  ...
  // Normalize historical settings into the packaged local runtime before any
  // provider-specific UI or fetch path can be used. This protects upgrades
  // from stale remote-provider rows without sending their credentials.
  useEffect(() => {
    if (modelConfig.provider === "builtin-ai") return;
    const localModel = builtinAiModels[0]?.name || "gemma3:1b";
    ...
    setApiKey(null);
    setError(
      "Legacy remote providers are disabled; using the packaged local model.",
    );
  }, [modelConfig.provider, builtinAiModels, setModelConfig]);
  ...
  const loadOpenRouterModels = async () => {
    if (modelConfig.provider !== "openrouter") return;
  ...
  const loadOpenAIModels = async (key: string | null) => {
    if (modelConfig.provider !== "openai") return;
    if (!key?.trim()) {
  ...
  const loadClaudeModels = async (key: string | null) => {
    if (modelConfig.provider !== "claude") return;
    if (!key?.trim()) {
  ...
  const loadGroqModels = async (key: string | null) => {
    if (modelConfig.provider !== "groq") return;
    if (!key?.trim()) {
                  ...
                  <div className="relative flex-1">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (
                          e.target.value.trim() !== lastFetchedEndpoint.trim()
                        ) {
                          setModels([]);
