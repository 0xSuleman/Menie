...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  const isPrivacyPaused = recordingState.isPrivacyPaused;
  ...
  const startRecordingAfterConsent = useCallback(async () => {
  ...
  const handleStartRecording = useCallback(async () => {
    if (!hasAcknowledgedRecordingNotice()) {
      setShowConsentNotice(true);
      return;
    }
    await startRecordingAfterConsent();
  }, [startRecordingAfterConsent]);

  const acknowledgeRecordingNotice = useCallback(() => {
    acknowledgeRecordingNotice();
    setShowConsentNotice(false);
    void startRecordingAfterConsent();
  }, [startRecordingAfterConsent]);
  ...
  // Meeting detection can request the same stop path as the visible Stop button.
  // Keeping this here ensures media finalization and post-processing are preserved.
  useEffect(() => {
    const stopFromMeetingDetection = () => {
      void handleStopRecording();
    };
    window.addEventListener(
      "stop-recording-from-meeting-detection",
      stopFromMeetingDetection,
    );
    return () =>
  ...
  const handlePrivacyPauseRecording = useCallback(async () => {
    if (!isRecording || isPaused || isPausing) return;
    setIsPausing(true);
    try {
      await invoke("privacy_pause_recording");
    } catch (error) {
      console.error("Failed to activate Privacy Pause:", error);
      alert(
        "Privacy Pause could not be activated. Audio capture is still active.",
      );
    } finally {
      setIsPausing(false);
    }
  ...
  const handleAddMarker = useCallback(async () => {
    ...
    const text = window.prompt("Add a note for this moment in the recording:");
    if (!text?.trim()) return;
    try {
      const marker = await invoke<{ offset_seconds: number; text: string }>(
        "add_recording_marker",
        { text },
      );
      setLastMarker(`${formatTime(marker.offset_seconds)} — ${marker.text}`);
    } catch (error) {
      console.error("Failed to add recording marker:", error);
  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col space-y-2">
              ...
              {showPlayback ? (
                <>
                  <button
                    ...
                    className="brand-gradient brand-glow w-10 h-10 flex items-center justify-center rounded-full text-white transition-transform hover:scale-105"
                  >
                    <Mic size={16} />
                  </button>
                        ...
                        <button
                          ...
                          className={`w-12 h-12 flex items-center justify-center ${
                            isStarting || isProcessing || isValidatingModel
                              ? "bg-gray-400"
                              : "brand-gradient brand-glow hover:scale-105"
                    ...
                    <>
                      ...
                      {!isPaused && (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button
                              onClick={handlePrivacyPauseRecording}
                              disabled={isPausing || isStopping}
                              className="w-10 h-10 flex items-center justify-center rounded-full border-2 border-amber-400 bg-amber-50 text-amber-800 hover:bg-amber-100 transition-colors"
                              aria-label="Activate Privacy Pause"
                            >
                              <ShieldAlert size={16} />
                            </button>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p>
                              Privacy Pause — stop saving and transcribing new
                              audio
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            className="w-10 h-10 flex items-center justify-center rounded-full border-2 border-blue-300 bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors"
                            aria-label="Add timestamped recording note"
                          >
                            <Flag size={16} />
                          </button>
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>Add timestamped note</p>
                        </TooltipContent>
                      </Tooltip>
        ...
        {isPrivacyPaused && (
          <div
            className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-center text-xs font-medium text-amber-900"
            role="status"
          >
            Privacy Pause is active. Audio captured after this point is not
            saved or transcribed. Resume when it is safe to continue.
          </div>
        )}
        {isRecording && lastMarker && (
          <div
            className="mt-2 max-w-sm truncate text-center text-xs text-blue-700"
            role="status"
          >
            Marker added: {lastMarker}
          </div>
        )}
        ...
        {showConsentNotice && (
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="recording-notice-title"
          >
            <div className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
              <h2
                id="recording-notice-title"
                className="text-lg font-semibold text-gray-900"
              >
                Before recording
              </h2>
              <p className="mt-2 text-sm text-gray-700">
                Tell participants that recording is taking place and obtain any
                consent required for your meeting and location. Menie cannot
                determine the rules that apply to you.
              </p>
              <p className="mt-2 text-xs text-gray-500">
                This acknowledgement is stored only on this device and can be
                revisited by clearing local app settings.
              </p>
