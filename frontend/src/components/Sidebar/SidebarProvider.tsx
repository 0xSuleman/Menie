...
interface SidebarContextType {
  ...
  trashedMeetings: CurrentMeeting[];
...
export function SidebarProvider({ children }: { children: React.ReactNode }) {
  ...
  const [trashedMeetings, setTrashedMeetings] = useState<CurrentMeeting[]>([]);
  const [archivedMeetings, setArchivedMeetings] = useState<CurrentMeeting[]>(
    [],
  );
  ...
  const fetchMeetings = React.useCallback(async () => {
    if (serverAddress) {
      try {
        ...
        const trashed = (await invoke("api_get_trashed_meetings")) as Array<{
          id: string;
          title: string;
        }>;
        setTrashedMeetings(
          trashed.map((meeting) => ({ id: meeting.id, title: meeting.title })),
        );
        const archived = (await invoke("api_get_archived_meetings")) as Array<{
          id: string;
          title: string;
        }>;
        setArchivedMeetings(
          archived.map((meeting) => ({ id: meeting.id, title: meeting.title })),
        );
  ...
  const unassignedMeetings = meetings.filter(
    (meeting) => !meeting.project?.trim(),
  );
  const projectFolders = Array.from(
    new Set(
      meetings
        .map((meeting) => meeting.project?.trim())
    ...
    .map((project) => ({
      ...
      children: meetings
        .filter((meeting) => meeting.project?.trim() === project)
        .map((meeting) => ({
          id: meeting.id,
          title: meeting.title,
          type: "file" as const,
      ...
      ? [
          {
            ...
            title: "Archive",
            type: "folder" as const,
            children: archivedMeetings.map((meeting) => ({
              id: meeting.id,
              title: meeting.title,
              type: "file" as const,
            })),
      ...
      ? [
          {
            ...
            title: "Trash",
            type: "folder" as const,
            children: trashedMeetings.map((meeting) => ({
              id: meeting.id,
              title: meeting.title,
              type: "file" as const,
            })),
  ...
  }, [pathname, meetings, archivedMeetings, trashedMeetings]);

  // Update sidebar items when meetings change
  useEffect(() => {
    setSidebarItems(baseItems);
  }, [meetings, archivedMeetings, trashedMeetings]);
  ...
  const startSummaryPolling = React.useCallback(
    (meetingId: string, processId: string, onUpdate: (result: any) => void) => {
      ...
      console.log(
        `📊 Starting polling for meeting ${meetingId}, process ${processId}`,
      );
      ...
      const pollInterval = setInterval(async () => {
        ...
        if (pollCount >= MAX_POLLS) {
          console.warn(
            `⏱️ Polling timeout for ${meetingId} after ${MAX_POLLS} iterations`,
          );
        ...
        try {
          ...
          if (
            ...
            console.log(
              `Polling completed for ${meetingId}, status: ${result.status}`,
            );
            clearInterval(pollInterval);
            setActiveSummaryPolls((prev) => {
              const next = new Map(prev);
            ...
            console.log(
              `Process completed or not found for ${meetingId}, stopping poll`,
            );
            clearInterval(pollInterval);
            setActiveSummaryPolls((prev) => {
              const next = new Map(prev);
