...
export default function AnalyticsProvider({
  ...
  useEffect(() => {
    ...
    const initAnalytics = async () => {
      ...
      // This distribution has no telemetry transport. Revoke any legacy
      // opt-in persisted by an older build before React can initialize it.
      if (analyticsOptedIn === true) {
        analyticsOptedIn = false;
        await store.set("analyticsOptedIn", false);
        await store.save();
      }
