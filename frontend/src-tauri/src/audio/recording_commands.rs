...
use super::recording_state::RecordingMarker;
use super::{
...
/// Privacy-safe capture diagnostics. This contains counters and device
/// presence only; it never includes audio, transcript text, or device names.
#[derive(Debug, Serialize, Clone)]
pub struct CaptureHealth {
    pub is_recording: bool,
    pub is_paused: bool,
    pub is_privacy_paused: bool,
    pub active_sources: Vec<String>,
    pub active_stream_count: usize,
    pub chunks_processed: u64,
    pub dropped_chunks: u64,
    pub processing_lag_ms: Option<u64>,
    pub capture_error_count: u32,
    pub reconnecting: bool,
    pub buffer_pressure: String,
}

#[cfg(test)]
mod capture_health_tests {
    use super::CaptureHealth;

    #[test]
    fn capture_health_serialization_is_content_free() {
        let payload = serde_json::to_value(CaptureHealth {
            is_recording: false,
            is_paused: false,
            is_privacy_paused: false,
            active_sources: vec!["microphone".to_string()],
            active_stream_count: 1,
            chunks_processed: 3,
            dropped_chunks: 0,
            processing_lag_ms: Some(12),
            capture_error_count: 0,
            reconnecting: false,
            buffer_pressure: "Unbounded local queue".to_string(),
        })
        .expect("capture-health payload should serialize");
        let serialized = payload.to_string();
        assert!(!serialized.contains("transcript"));
        assert!(!serialized.contains("audio"));
        assert!(!serialized.contains("device_name"));
        assert!(!serialized.contains("meeting"));
    }
}

async fn record_only_enabled<R: Runtime>(app: &AppHandle<R>) -> bool {
    super::recording_preferences::load_recording_preferences(app)
        .await
        .map(|preferences| preferences.record_only)
        .unwrap_or(false)
}

async fn warn_for_low_recording_storage<R: Runtime>(app: &AppHandle<R>) {
    let preferences = match super::recording_preferences::load_recording_preferences(app).await {
        Ok(preferences) => preferences,
        Err(error) => {
            warn!("Could not load recording storage preferences: {}", error);
            return;
        }
    };
    match super::recording_preferences::recording_storage_status(&preferences.save_folder) {
        Ok(status) if status.is_warning => {
            let _ = app.emit("recording-storage-warning", &status);
            warn!(
                "Low recording storage: {} bytes available at {}",
                status.available_bytes, status.destination
            );
        }
        Ok(_) => {}
        Err(error) => warn!("Could not inspect recording storage: {}", error),
    }
}
...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    warn_for_low_recording_storage(&app).await;

    let record_only = record_only_enabled(&app).await;
    ...
    let task_handle = if record_only {
...
pub async fn stop_recording<R: Runtime>(
    ...
    let mut recording_markers = Vec::new();
    let (meeting_folder, meeting_name) = if let Some(mut manager) = manager_for_cleanup {
        ...
        recording_markers = manager.get_state().get_markers();
...
/// Add a local, recording-relative marker without interrupting capture.
#[tauri::command]
pub async fn add_recording_marker(text: String) -> Result<RecordingMarker, String> {
...
/// Activate Privacy Pause. This is intentionally separate from ordinary pause
/// so the UI can continuously disclose that no post-boundary audio is stored.
#[tauri::command]
pub async fn privacy_pause_recording<R: Runtime>(app: AppHandle<R>) -> Result<(), String> {
    if !IS_RECORDING.load(Ordering::SeqCst) {
        return Err("No recording is currently active".to_string());
    }
    ...
    manager
        .privacy_pause_recording()
        .map_err(|e| e.to_string())?;

    app.emit(
        "recording-privacy-paused",
        serde_json::json!({
            "message": "Privacy Pause active. Audio captured after this point is not saved or transcribed."
        }),
    )
    .map_err(|e| e.to_string())?;
    crate::tray::update_tray_menu(&app);
...
pub async fn get_recording_state() -> serde_json::Value {
    ...
    if let Some(manager) = manager_guard.as_ref() {
        serde_json::json!({
            ...
            "is_privacy_paused": manager.is_privacy_paused(),
...
/// Return local capture counters and source presence for the optional
/// diagnostics view. Device names and meeting content are deliberately
/// excluded. The current queue is unbounded, so pressure is reported
/// honestly rather than represented as a fabricated percentage.
#[tauri::command]
pub async fn get_capture_health() -> CaptureHealth {
    let manager_guard = RECORDING_MANAGER.lock().unwrap();
    let Some(manager) = manager_guard.as_ref() else {
        return CaptureHealth {
            ...
            capture_error_count: 0,
            reconnecting: false,
            buffer_pressure: "No active capture queue".to_string(),
        };
    };
    ...
    if state.get_microphone_device().is_some() {
        active_sources.push("microphone".to_string());
    }
    if state.get_system_device().is_some() {
        active_sources.push("system_audio".to_string());
    }
    CaptureHealth {
        is_recording: IS_RECORDING.load(Ordering::SeqCst),
        is_paused: manager.is_paused(),
        is_privacy_paused: manager.is_privacy_paused(),
        active_sources,
        active_stream_count: manager.active_stream_count(),
        chunks_processed: stats.chunks_processed,
        dropped_chunks: stats.dropped_chunks,
        processing_lag_ms: stats
            .last_activity
            .map(|last| last.elapsed().as_millis().min(u128::from(u64::MAX)) as u64),
        capture_error_count: state.get_error_count(),
        reconnecting: manager.is_reconnecting(),
        buffer_pressure: "Unbounded local queue; pressure percentage unavailable".to_string(),
    }
}
