...
pub fn validate_audio_file(path: &Path) -> Result<AudioFileInfo> {
    // Check file exists
    if !path.exists() {
        return Err(anyhow!("Selected audio file does not exist"));
    }
...
async fn run_import<R: Runtime>(
    ...
    // Do not log user-controlled meeting titles or source paths. These are
    // local workflow data, not diagnostics.
    info!(
        "Starting local audio import (language configured: {}, model configured: {}, provider configured: {})",
        language.is_some(),
        model.is_some(),
        provider.is_some()
    );
    ...
    info!("Copied imported audio into the local meeting workspace");
...
async fn create_meeting_with_transcripts(
    ...
    // Insert meeting
    sqlx::query(
        "INSERT INTO meetings (id, title, created_at, updated_at, folder_path, project)
         VALUES (?, ?, ?, ?, ?, ?)",
    )
    .bind(&meeting_id)
    .bind(title)
...
fn write_import_metadata(
    ...
    info!("Wrote local imported-meeting metadata");
    Ok(())
}
...
pub async fn select_and_validate_audio_command<R: Runtime>(
    ...
    match file_path {
        Some(path) => {
            ...
            info!("User selected an audio file for local import");
...
pub async fn start_import_audio_command<R: Runtime>(
    ...
    tauri::async_runtime::spawn(async move {
        let result =
            start_import(app, source_path, title, project, language, model, provider).await;

        if let Err(e) = result {
            // The detailed error is returned through the local event channel;
            // avoid duplicating user paths or titles in persistent logs.
            let _ = e;
