...
pub struct RecordingStats {
    ...
    /// Chunks that could not be handed to the local processing queue.
    pub dropped_chunks: u64,
    pub total_duration: f64,
    pub last_activity: Option<Instant>,
}

/// A user-created, recording-relative note. Marker text is kept local and is
/// emitted with the finalized meeting so it can be persisted with its ID.
...
pub struct RecordingState {
    ...
    // Privacy pause is a stricter, user-visible reason for pausing capture. It
    // still uses the same capture gate, which discards chunks before they enter
    // the persistence/transcription pipeline.
    is_privacy_paused: AtomicBool,
    ...
    markers: Mutex<Vec<RecordingMarker>>,
}
...
impl RecordingState {
    pub fn new() -> Arc<Self> {
        Arc::new(Self {
            ...
            is_privacy_paused: AtomicBool::new(false),
    ...
    pub fn start_recording(&self) -> Result<()> {
        ...
        self.is_privacy_paused.store(false, Ordering::SeqCst);
    ...
    pub fn add_marker(&self, text: String) -> Result<RecordingMarker> {
        if !self.is_recording() {
            return Err(anyhow::anyhow!("Cannot add a marker when not recording"));
        }
        let text = text.trim();
        if text.is_empty() || text.chars().count() > 500 {
            return Err(anyhow::anyhow!(
                "Marker text must contain 1 to 500 characters"
            ));
        }
        let marker = RecordingMarker {
            offset_seconds: self.get_recording_duration().unwrap_or(0.0).max(0.0),
            text: text.to_string(),
        };
    ...
    /// Stop accepting audio at the capture boundary for a user-requested
    /// privacy pause. Chunks observed after this call are discarded before
    /// they can reach the media saver or live transcription pipeline.
    pub fn privacy_pause_recording(&self) -> Result<()> {
        if !self.is_recording() {
            return Err(anyhow::anyhow!(
                "Cannot activate Privacy Pause when not recording"
            ));
        }
        if self.is_paused() {
            return Err(anyhow::anyhow!("Recording is already paused"));
        ...
        self.is_paused.store(true, Ordering::SeqCst);
        self.is_privacy_paused.store(true, Ordering::SeqCst);
        *self.pause_start.lock().unwrap() = Some(Instant::now());
        log::info!("Recording privacy-paused; incoming chunks will be discarded");
        Ok(())
    }
    ...
    pub fn is_privacy_paused(&self) -> bool {
        self.is_privacy_paused.load(Ordering::SeqCst)
    }
    ...
    pub fn send_audio_chunk(&self, chunk: AudioChunk) -> Result<()> {
        ...
        if let Some(sender) = self.audio_sender.lock().unwrap().as_ref() {
            sender.send(chunk).map_err(|_| {
                self.stats.lock().unwrap().dropped_chunks += 1;
...
mod tests {
    ...
    #[test]
    fn a_new_recording_resets_pause_accounting_from_the_previous_session() {
        let state = RecordingState::new();
        state.start_recording().unwrap();
        state.pause_recording().unwrap();
        std::thread::sleep(std::time::Duration::from_millis(2));
        state.resume_recording().unwrap();
        assert!(state.get_total_pause_duration() > 0.0);
        ...
        assert!(state.get_current_pause_duration().is_none());
    }

    #[test]
    fn recording_markers_are_relative_bounded_local_notes() {
        let state = RecordingState::new();
        assert!(state.add_marker("before start".to_string()).is_err());
        state.start_recording().unwrap();
        let marker = state.add_marker("Decision noted".to_string()).unwrap();
        assert!(marker.offset_seconds >= 0.0);
        assert_eq!(state.get_markers(), vec![marker]);
        assert!(state.add_marker(" ".to_string()).is_err());
        state.stop_recording();
        state.start_recording().unwrap();
        assert!(state.get_markers().is_empty());
    }

    #[test]
    fn privacy_pause_discards_capture_and_is_cleared_on_resume() {
        let state = RecordingState::new();
        state.start_recording().unwrap();
        let (sender, mut receiver) = mpsc::unbounded_channel();
        state.set_audio_sender(sender);

        state.privacy_pause_recording().unwrap();
        assert!(state.is_paused());
        assert!(state.is_privacy_paused());
    ...
    #[test]
    fn recoverable_errors_keep_capture_active_until_the_retry_budget_is_exhausted() {
        let state = RecordingState::new();
        state.start_recording().unwrap();

        for _ in 0..9 {
            state.report_error(AudioError::DeviceDisconnected);
            assert!(state.is_recording());
        }

        state.report_error(AudioError::DeviceDisconnected);
        assert!(!state.is_recording());
        assert_eq!(state.get_recoverable_error_count(), 10);
    }

    #[test]
    fn non_recoverable_errors_stop_capture_immediately() {
        let state = RecordingState::new();
        state.start_recording().unwrap();
