...
impl RecordingManager {
    ...
    /// Pause capture with the explicit privacy reason. The state object gates
    /// chunks before the saver and transcription workers receive them.
    pub fn privacy_pause_recording(&self) -> Result<()> {
        info!("Privacy-pausing recording");
        self.state.privacy_pause_recording()
    }
    ...
    pub async fn handle_device_disconnect(
        ...
        if let Some(device) = device {
            ...
            // Do not create a silent, apparently-live interval while a selected
            // device is absent. The successful reconnect path resumes capture.
            if self.state.is_recording() && !self.state.is_paused() {
                if let Err(error) = self.state.pause_recording() {
                    warn!(
                        "Failed to pause recording during device reconnect: {}",
                        error
                    );
                }
            }
