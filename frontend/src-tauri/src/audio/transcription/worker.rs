...
/// Attribute the local microphone before any probabilistic diarization runs.
/// This is deterministic provenance, not an inferred identity: audio captured
/// from the user's mic is "Me" and system/mixed audio remains a generic remote
/// participant label until local diarization has sufficient evidence.
fn source_label(device_type: &RecordingDeviceType) -> &'static str {
    match device_type {
        RecordingDeviceType::Microphone => "Me",
        RecordingDeviceType::System => "Remote participant",
    }
}
                ...
                loop {
                    ...
                    match chunk {
                        Some(chunk) => {
                            ...
                            let source = source_label(&chunk.device_type).to_string();
...
mod tests {
    ...
    #[test]
    fn source_track_attribution_is_deterministic_and_conservative() {
        assert_eq!(source_label(&RecordingDeviceType::Microphone), "Me");
        assert_eq!(
            source_label(&RecordingDeviceType::System),
            "Remote participant"
        );
    }
}
