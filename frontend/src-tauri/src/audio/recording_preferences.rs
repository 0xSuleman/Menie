...
pub struct RecordingPreferences {
    ...
    /// Save media without starting live local speech recognition.
    #[serde(default)]
...
/// Storage information for the configured recording destination. The time
/// estimate reserves enough space for two uncompressed 48kHz/32-bit tracks,
/// so it warns before a recording can become unsafe.
#[derive(Debug, Serialize, Clone)]
pub struct RecordingStorageStatus {
    pub available_bytes: u64,
    pub warning_threshold_bytes: u64,
    pub estimated_safe_minutes: u64,
    pub is_warning: bool,
    pub destination: String,
}

const STORAGE_WARNING_THRESHOLD_BYTES: u64 = 2 * 1024 * 1024 * 1024;
const CONSERVATIVE_BYTES_PER_MINUTE: u64 = 64 * 1024 * 1024;

pub fn recording_storage_status(destination: &PathBuf) -> Result<RecordingStorageStatus> {
    ...
    let disk = disks
        ...
        .ok_or_else(|| anyhow::anyhow!("No mounted disk found for recording destination"))?;
    let available_bytes = disk.available_space();

    Ok(RecordingStorageStatus {
        available_bytes,
        warning_threshold_bytes: STORAGE_WARNING_THRESHOLD_BYTES,
        estimated_safe_minutes: available_bytes / CONSERVATIVE_BYTES_PER_MINUTE,
        is_warning: available_bytes < STORAGE_WARNING_THRESHOLD_BYTES,
...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\menie-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("menie-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("menie-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/menie-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("menie-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/menie-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("menie-recordings")
    }
...
#[tauri::command]
pub async fn get_recording_storage_status<R: Runtime>(
    app: AppHandle<R>,
) -> Result<RecordingStorageStatus, String> {
    let preferences = load_recording_preferences(&app)
        .await
        .map_err(|error| format!("Failed to load recording preferences: {error}"))?;
    recording_storage_status(&preferences.save_folder)
        .map_err(|error| format!("Failed to inspect recording storage: {error}"))
}
...
#[cfg(test)]
mod tests {
    use super::RecordingPreferences;

    #[test]
    fn record_only_is_disabled_by_default() {
        assert!(!RecordingPreferences::default().record_only);
    }
}
