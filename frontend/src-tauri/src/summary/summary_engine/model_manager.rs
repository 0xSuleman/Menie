...
fn allowed_model_host(host: Option<&str>) -> bool {
    matches!(host, Some("huggingface.co")) || host.is_some_and(|value| value.ends_with(".hf.co"))
}

fn integrity_path(path: &PathBuf) -> PathBuf {
...
impl ModelManager {
                ...
                match fs::metadata(&model_path).await {
                    Ok(metadata) => {
                        ...
                        if file_size_mb >= expected_min && file_size_mb <= expected_max {
                            match self.verify_or_record_integrity(&model_path).await {
                                Ok(()) => {
    ...
    pub async fn download_model_detailed(
        ...
        if file_path.exists() {
            if let Ok(metadata) = fs::metadata(&file_path).await {
                ...
                if file_size_mb >= expected_min && file_size_mb <= expected_max {
                    if let Err(error) = self.verify_or_record_integrity(&file_path).await {
                        log::warn!(
                            "Model '{}' failed integrity verification: {}; re-downloading",
                            model_name,
                            error
                        );
                        let _ = fs::remove_file(&file_path).await;
                        let _ = fs::remove_file(integrity_path(&file_path)).await;
                    } else {
                        log::info!(
        ...
        let model_url = url::Url::parse(&model_def.download_url)
            .map_err(|error| anyhow!("Invalid built-in model URL: {error}"))?;
        if model_url.scheme() != "https" || !allowed_model_host(model_url.host_str()) {
            return Err(anyhow!(
                "Built-in model URL is outside the approved HTTPS model host"
            ));
        }

        // Hugging Face legitimately redirects to its CDN; permit only the
        // pinned Hugging Face host family and reject arbitrary destinations.
        let client = Client::builder()
            .redirect(reqwest::redirect::Policy::custom(|attempt| {
                if allowed_model_host(attempt.url().host_str()) {
                    attempt.follow()
                } else {
                    attempt.stop()
                }
        ...
        let integrity_result = async {
            let digest = self.sha256_file(&file_path).await?;
            self.write_integrity_sidecar(&file_path, &digest, &model_def.gguf_file)
                .await
        }
        .await;
        if let Err(error) = integrity_result {
            log::error!("Model '{}' integrity record failed: {}", model_name, error);
            let mut models = self.available_models.write().await;
            if let Some(model_info) = models.get_mut(model_name) {
                model_info.status = ModelStatus::Error(format!("Integrity record failed: {error}"));
            }
            drop(models);
    ...
    async fn sha256_file(&self, path: &PathBuf) -> Result<String> {
        use tokio::io::AsyncReadExt;
        let mut file = fs::File::open(path).await?;
    ...
    async fn verify_or_record_integrity(&self, path: &PathBuf) -> Result<()> {
        let digest = self.sha256_file(path).await?;
        let sidecar = integrity_path(path);
        match fs::read_to_string(&sidecar).await {
            Ok(contents) => {
                let recorded = contents.split_whitespace().next().unwrap_or_default();
                if recorded != digest {
                    return Err(anyhow!("SHA-256 digest mismatch"));
                }
            }
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                self.write_integrity_sidecar(
                    path,
                    &digest,
    ...
    async fn write_integrity_sidecar(
        &self,
        path: &PathBuf,
        digest: &str,
        display_name: &str,
        ...
        let sidecar = integrity_path(path);
        let temporary = PathBuf::from(format!("{}.tmp", sidecar.display()));
        fs::write(&temporary, format!("{}  {}\n", digest, display_name)).await?;
        fs::rename(temporary, sidecar).await?;
        Ok(())
    }
...
#[cfg(test)]
mod tests {
    use super::allowed_model_host;

    #[test]
    fn model_redirect_policy_allows_only_huggingface_hosts() {
        assert!(allowed_model_host(Some("huggingface.co")));
        assert!(allowed_model_host(Some("cdn-lfs-us-1.hf.co")));
        assert!(!allowed_model_host(Some("example.com")));
        assert!(!allowed_model_host(None));
    }
}
