...
fn build_summary_result_json(
    ...
    build_summary_result_json_with_evidence(
        final_markdown,
        english_markdown,
        source,
        output_language,
...
fn build_summary_result_json_with_evidence(
    final_markdown: &str,
    english_markdown: &str,
    source: SummaryCacheSource,
    output_language: Option<&str>,
    evidence_segments: &[crate::summary::outcomes::EvidenceSegment],
) -> serde_json::Value {
    let mut outcomes = crate::summary::outcomes::extract_structured_outcomes(final_markdown);
    crate::summary::outcomes::attach_exact_evidence(&mut outcomes, evidence_segments);
    serde_json::json!({
    ...
    pub async fn process_transcript_background<R: tauri::Runtime>(
        ...
        match result {
            Ok((final_markdown, english_markdown, num_chunks)) => {
                ...
                let evidence_segments = sqlx::query_as::<_, (String, Option<f64>, Option<f64>)>(
                    "SELECT transcript, audio_start_time, audio_end_time FROM transcripts WHERE meeting_id = ? ORDER BY audio_start_time",
                )
                .bind(&meeting_id)
                .fetch_all(&pool)
                .await
                .unwrap_or_else(|query_error| {
                    warn!("Failed to load transcript evidence for {}: {}", meeting_id, query_error);
                    Vec::new()
                })
                .into_iter()
                .map(|(text, start_time, end_time)| crate::summary::outcomes::EvidenceSegment {
                    text,
                ...
                let result_json = build_summary_result_json_with_evidence(
                    &final_markdown,
                    &english_markdown,
                    cache_source,
                    summary_language.as_deref(),
