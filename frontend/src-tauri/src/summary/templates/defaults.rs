...
mod tests {
    ...
    #[test]
    fn standard_meeting_keeps_reviewable_outcome_sections() {
        let template: crate::summary::templates::Template =
            serde_json::from_str(STANDARD_MEETING).expect("standard template is valid");
        let titles: Vec<_> = template
            .sections
            .iter()
        ...
        for required in [
            ...
            assert!(
                ...
                "missing outcome section: {required}"
            );
        }
    }
}
