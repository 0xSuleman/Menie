...
pub async fn init_analytics() -> Result<(), String> {
    // The local-only distribution never creates a telemetry client.  Keep this
    // command as a harmless compatibility shim for older UI code, rather than
    // allowing a preference toggle to re-enable a network destination.
