...
/// A transparent local model choice presented during first-run setup.
/// The recommendation is derived only from local hardware characteristics.
...
fn local_model_profiles(profile: &HardwareProfile) -> Vec<LocalModelProfile> {
    let recommended_id = match profile.performance_tier {
        PerformanceTier::Low => "gemma3:1b",
        PerformanceTier::Medium => "qwen3.5:2b",
        PerformanceTier::High | PerformanceTier::Ultra => "qwen3.5:4b",
    };
    let acceleration = if profile.has_gpu_acceleration {
        "GPU acceleration is available"
    } else {
        "this device will use CPU processing"
    };
    let reason = format!(
        "{} GB memory, {} CPU cores, and {}; this profile best balances local quality and responsiveness on this device.",
        profile.memory_gb, profile.cpu_cores, acceleration
    );
    ...
    [
        (
            "gemma3:1b",
            "Efficient",
            "Fast local summaries for constrained hardware.",
            4,
        ),
        (
            "qwen3.5:2b",
            "Balanced",
            "Recommended quality and speed for most laptops.",
            8,
        ),
        (
            "qwen3.5:4b",
            "High Accuracy",
            "Best local meeting analysis for capable computers.",
            16,
        ),
    ]
    .into_iter()
    .map(
        |(id, name, description, minimum_memory_gb)| LocalModelProfile {
            id: id.to_string(),
            name: name.to_string(),
            description: description.to_string(),
            minimum_memory_gb,
            recommended: id == recommended_id,
            recommendation_reason: if id == recommended_id {
                reason.clone()
            } else {
                String::new()
            },
...
#[tauri::command]
pub fn get_local_model_profiles() -> Vec<LocalModelProfile> {
    local_model_profiles(HardwareProfile::detect())
}
...
mod tests {
    ...
    #[test]
    fn recommends_a_single_profile_for_each_hardware_tier() {
        let profile = HardwareProfile {
            cpu_cores: 4,
            has_gpu_acceleration: false,
            gpu_type: crate::audio::hardware_detector::GpuType::None,
            memory_gb: 8,
            performance_tier: PerformanceTier::Low,
        };
        let profiles = local_model_profiles(&profile);
        assert_eq!(profiles.iter().filter(|item| item.recommended).count(), 1);
        assert_eq!(
            profiles.iter().find(|item| item.recommended).unwrap().id,
            "gemma3:1b"
        );
    }
}
