...
#[command]
pub async fn whisper_set_vocabulary(terms: Vec<String>) -> Result<Vec<String>, String> {
    let engine = {
    ...
    let engine = match engine {
        ...
        None => {
            ...
            guard
                .as_ref()
                .cloned()
                .ok_or_else(|| "Whisper engine is not initialized".to_string())?
        }
    };
    engine
        .set_vocabulary(terms)
        .await
        .map_err(|error| format!("Failed to set Whisper vocabulary: {error}"))
}
