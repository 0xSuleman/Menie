...
pub struct WhisperEngine {
    ...
    vocabulary_prompt: Arc<RwLock<Option<String>>>,
}

impl WhisperEngine {
    fn integrity_path(path: &std::path::Path) -> PathBuf {
        PathBuf::from(format!("{}.sha256", path.display()))
    }

    async fn verify_or_record_integrity(path: &std::path::Path) -> Result<()> {
        ...
        let sidecar = Self::integrity_path(path);
        match fs::read_to_string(&sidecar).await {
            Ok(recorded) if recorded.trim() != digest => Err(anyhow!(
                "Whisper model integrity mismatch for {}",
                path.display()
            )),
            Ok(_) => Ok(()),
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        ...
        let engine = Self {
            ...
            vocabulary_prompt: Arc::new(RwLock::new(None)),
        };

        Ok(engine)
    }

    /// Set bounded, local vocabulary hints used as Whisper's initial prompt.
    /// These hints influence decoding only; they are never sent outside the process.
    pub async fn set_vocabulary(&self, terms: Vec<String>) -> Result<Vec<String>> {
        ...
        let prompt = if normalized.is_empty() {
            None
        } else {
            Some(format!("Domain vocabulary: {}", normalized.join(", ")))
        };
        *self.vocabulary_prompt.write().await = prompt;
        Ok(normalized)
    }

    async fn apply_vocabulary_prompt(&self, params: &mut FullParams<'_, '_>) {
        if let Some(prompt) = self.vocabulary_prompt.read().await.as_deref() {
                ...
                match std::fs::metadata(&model_path) {
                    Ok(metadata) => {
                        ...
                        if file_size_mb >= expected_min_size_mb && file_size_mb > 1 {
                            // File size looks good; validate the local integrity sidecar and format.
                            match Self::verify_or_record_integrity(&model_path).await {
                                Ok(()) => match self.validate_model_file(&model_path).await {
                                    Ok(_) => ModelStatus::Available,
                                    Err(_) => {
                                        log::warn!("Model file {} has correct size but appears corrupted (failed validation)", filename);
    ...
    pub async fn transcribe_audio_with_confidence(
        ...
        self.apply_vocabulary_prompt(&mut params).await;
    ...
    pub async fn delete_model(&self, model_name: &str) -> Result<String> {
        ...
        match &model_info.status {
            ModelStatus::Corrupted {
                ...
                let _ = fs::remove_file(Self::integrity_path(&model_info.path)).await;
    ...
    pub async fn cancel_download(&self, model_name: &str) -> Result<()> {
        ...
        let _ = fs::remove_file(Self::integrity_path(&file_path)).await;
