...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.menie\meeting_minutes.db",
        "$env:USERPROFILE\Documents\menie\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/menie-backend/*/backend/meeting_minutes.db"
    }
