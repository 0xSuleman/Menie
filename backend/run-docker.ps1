...
$AppProjectName = "menie-backend"
$AppContainerName = "menie-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "menie-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server menie-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server menie-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "menie-backend"
        "backend" = "menie-backend"
        "meeting" = "menie-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "menie-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, menie-backend"
        ...
        $logArgs += @("whisper-server", "menie-backend")
    }
