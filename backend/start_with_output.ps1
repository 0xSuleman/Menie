...
Write-Host "====================================="
Write-Host "Menie Backend Startup"
Write-Host "====================================="
...
if (-not (Test-Path "whisper-server-package\whisper-server.exe")) {
    ...
    try {
        ...
        $apiUrl = "https://api.github.com/repos/0xSuleman/Menie/releases/latest"
        ...
        $downloadUrl = "https://github.com/0xSuleman/Menie/releases/download/$tagName/whisper-server.exe"
        ...
        try {
            $allReleasesUrl = "https://api.github.com/repos/0xSuleman/Menie/releases"
            $headers = @{"User-Agent" = "PowerShell-Script"}
            ...
            if ($releases.Count -gt 0) {
                ...
                $altDownloadUrl = "https://github.com/0xSuleman/Menie/releases/download/$latestTag/whisper-server.exe"
            ...
            Write-Host "https://github.com/0xSuleman/Menie/releases"
...
Write-Host "====================================="
Write-Host "Starting Menie Backend"
Write-Host "====================================="
...
# Check if menie-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for menie-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\menie-frontend\menie-frontend.exe",
    "$env:LOCALAPPDATA\Programs\menie\menie-frontend.exe",
    "$env:ProgramFiles\menie-frontend\menie-frontend.exe",
    "${env:ProgramFiles(x86)}\menie-frontend\menie-frontend.exe",
    "$env:APPDATA\menie-frontend\menie-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("menie-frontend.exe", "menie.exe")
...
if ($frontendInstalled) {
    Write-Host "Menie frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the Menie frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching Menie frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "Menie frontend launched successfully."
        }
    }
} else {
    Write-Host "Menie frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the Menie frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if menie is now installed and launch it
