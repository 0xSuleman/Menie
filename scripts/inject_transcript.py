...
Injects CSV-based transcript data into the Menie SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "Menie"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "Menie"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "Menie"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "Menie"
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Menie database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure Menie has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Menie sidebar.")
