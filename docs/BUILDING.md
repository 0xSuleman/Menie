# Building Menie from Source

This guide provides detailed instructions for building Menie from source on different operating systems.
...
This guide helps you build Menie on Linux with **automatic GPU acceleration**. The build system detects your hardware and configures the best performance automatically.
...
```
src-tauri/target/release/bundle/appimage/Menie_<version>_amd64.AppImage
```
...
# Or from the repository root on Windows (sets the local no-bindgen environment)
.\frontend\run_app.ps1
